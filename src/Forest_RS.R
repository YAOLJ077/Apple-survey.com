...
library("magrittr")
library("checkmate")
forest_data<-read.csv(("/Users/yaolinjia/Desktop/apple/Apple-survey.com/data/Forest_RS.csv"),header=T)
#forest_data<-read.csv(("F:/R/Data_bank/Forest_RS.csv"),header=T)
#head(forest_data)
#View(forest_data)
forestplot(as.matrix(forest_data[,1:4]),mean=forest_data$OR,lower=forest_data$lower,upper=forest_data$upper,
           ...
           col=fpColors(box="royalblue",lines="darkblue",zero="black",axes = "black"),
